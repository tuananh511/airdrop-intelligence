...
### Phase 4 — Scorer (V2) + Gemini AI (V3) ✅ (xong)
- [x] `src/scorer/base.py`: interface `ProjectScorer` (abstract, `score(project) -> ProjectScore`).
- [x] `src/scorer/rule_based.py`: `RuleBasedScorer` - chấm điểm dựa trên độ đầy đủ dữ liệu (có reward/deadline/description rõ ràng, đang Ongoing hay đã Ended, chi phí thấp) - **không tốn API call**, chạy được kể cả khi `ENABLE_AI_SCORING=false`. Luôn chạy cho MỌI project đã merge (trong `main.py`).
- [x] `src/ai/base.py`: interface `AIProvider` (abstract, `evaluate(project) -> AIScore | None`) - thiết kế để sau này thêm `OpenRouterProvider`/`OpenAIProvider` chỉ cần implement interface này, không sửa `main.py`.
- [x] `src/ai/gemini_provider.py`: `GeminiProvider` dùng SDK `google-genai` chính thức (`client.models.generate_content(..., config=types.GenerateContentConfig(response_mime_type="application/json", response_schema=...))`) - đã verify cú pháp đúng với doc chính thức của Google (không phải đoán). Lỗi bất kỳ (network, JSON không hợp lệ, thiếu field bắt buộc) đều trả về `None`, KHÔNG raise - AI là tính năng optional, không được làm chết pipeline chính.
- [x] Tích hợp vào `main.py`: rule-based score chạy cho **mọi** project đã merge (rẻ, luôn bật); AI score CHỈ chạy cho project **mới** (sau khi filter history) khi `ENABLE_AI_SCORING=true` - tiết kiệm API call, không đánh giá lại project đã biết.
- [x] `src/telegram/notifier.py` cập nhật: message ưu tiên hiển thị AI score (worth/risk/time/capital/summary) nếu có, fallback về điểm rule-based (`⭐ Điểm sơ bộ`) nếu AI chưa chạy.
- [x] `tests/test_scorer.py`, `tests/test_ai.py` (mock Gemini client hoàn toàn, không gọi API thật) — **33/33 test pass**.
- [x] Chạy thử `main.py` thật với `ENABLE_AI_SCORING=false` (mặc định) - xác nhận không phá vỡ hành vi Phase 3, `data/*.json` không bị ghi sai lệch.

**Xác nhận thêm về lỗi 403 (từ Phase 3):** đã đổi `User-Agent` sang giống Chrome nhưng vẫn bị 403 khi test ở sandbox này → nhiều khả năng là chặn theo IP (datacenter/cloud IP bị Cloudflare liệt vào danh sách chặn), không phải do User-Agent. **Cần test thật trên GitHub Actions runner** (IP khác) để biết chắc có bị chặn không.

### Bổ sung sau Phase 4 (theo yêu cầu user)
- [x] Đổi `GEMINI_MODEL` mặc định từ `gemini-2.0-flash` -> `gemini-3.1-flash-lite` (cả trong `.env.example` và default value trong `src/utils/config.py`).
- [x] Thêm `scripts/list_gemini_models.py`: liệt kê model Gemini đang khả dụng cho API key hiện tại (dùng `client.models.list()` + lọc model hỗ trợ `generateContent` - đã verify đúng API thật của SDK `google-genai`, không đoán mò). Dùng khi model bị Google retire/đổi tên trong tương lai - chạy script này để xem danh sách thật, rồi tự cập nhật `GEMINI_MODEL` trong `.env`.
- [x] `src/ai/gemini_provider.py`: khi lỗi Gemini API có dấu hiệu liên quan tới tên model (chứa "404"/"not found"), tự động log gợi ý chạy `scripts/list_gemini_models.py`.
- [x] Làm rõ với user: API key thật điền vào `.env` (đã gitignore từ Phase 1), KHÔNG điền vào `.env.example` (chỉ chứa placeholder, được commit công khai).

### Phase 5 — GitHub Actions + README hoàn chỉnh ✅ (xong)
- [x] `.github/workflows/crawl.yml`: cron `0 */2 * * *` (mỗi 2h) + `workflow_dispatch` (trigger thủ công để test). Cài `uv` qua `astral-sh/setup-uv@v8.1.0` (đã verify version thật qua doc chính thức - **lưu ý:** bản nháp đầu tiên dùng `@v3` là version KHÔNG tồn tại/lỗi thời, đã sửa lại trước khi giao). `permissions: contents: write` để job tự commit lại `data/projects.json` + `data/history.json` sau mỗi lần chạy (đây là cách "lưu state" duy nhất, không DB). Dùng `git diff --cached --quiet || git commit ...` để chỉ commit khi thực sự có thay đổi.
- [x] Secrets cần thêm trên GitHub (Settings → Secrets and variables → Actions): `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `GEMINI_API_KEY` (optional). Variables optional: `GEMINI_MODEL`, `ENABLE_AI_SCORING`.
- [x] `README.md` đầy đủ: giới thiệu, kiến trúc (folder tree + luồng chạy), cài đặt, bảng cấu hình `.env`, hướng dẫn setup GitHub Actions từng bước, mục "Giới hạn hiện tại" (nói rõ 3/4 crawler là stub + rủi ro 403), screenshot placeholder, roadmap, mục "Không làm" theo đúng scope spec ban đầu.
- [x] Toàn bộ 33 test vẫn pass sau khi thêm Phase 5.

**Tất cả 5 phase trong roadmap ban đầu đã hoàn thành.** Project ở trạng thái chạy được ngay sau khi:
1. Điền `.env` thật (hoặc GitHub Secrets) với Telegram token thật.
2. Push lên GitHub, chạy thử `workflow_dispatch` 1 lần thủ công để xác nhận không lỗi.
3. Theo dõi xem `airdrops.io` có bị 403 trên GitHub Actions runner không (việc còn nợ duy nhất mang tính "phải chờ test thật mới biết").
...
👉 **Roadmap V1-V3 đã xong.** Các lựa chọn tiếp theo (không bắt buộc, tùy user chọn):
   a) Test thật trên GitHub Actions (push code, thêm Secrets, chạy `workflow_dispatch` thủ công, xem log).
   b) Quay lại implement Galxe/Layer3/Zealy (cần DevTools-inspect network thật).
   c) Coi như xong V1, bắt đầu checklist dọn dẹp cuối dự án (xem section "🧹 CHECKLIST DỌN DẸP CUỐI DỰ ÁN" - phải hỏi xác nhận từng bước).
...
- **Theo dõi lỗi 403 Forbidden từ airdrops.io trên GitHub Actions thật** - đã đổi User-Agent nhưng vẫn bị 403 ở sandbox dev (nghi IP-based block) - cần xem log lần chạy Actions đầu tiên.
